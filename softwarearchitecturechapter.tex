...
\noindent In the previous scheme, there are no considerations for the weights associated with the degree of similarity between lyrics. In order to accommodate this we assume the existence of tag weights associated for tags in the song meta data. Let $T(S_i)$ be defined as the genre annotations for song $S_i$. Let $S_{sim} = sort_v^{desc}({S_i, | isSim(S_i, S_{new}) = v})$ be the rank ordered set of genre labels based on descending order of similarity values. Out of these top we choose the top k $S_{sim(k)} = first_{k} (S_{sim})$ songs. The genre estimation can then be defined as $T_{new} = \{\cup \ T(S_i) \ | \ S_i \in S_{sim(k)}\}$ \\\\
...
\item Modulo7 does not take into account varying time and/or key signatures. This is due to the fact that in most western music, these two global parameters stay constant for songs. Also Modulo7 does not take into account atonal music (as a key signature leads to transforms that are needed in similarity metrics)
